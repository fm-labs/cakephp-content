...
class OldPagesController extends ContentController
{
    ...
    public function relatedPageMeta($id = null)
    {
        ...
        //$this->set('template', 'related_page_meta');
        $this->render('related_page_meta');
    }
    ...
    public function relatedContentModules($id = null)
    {
        ...
        //$sectionsModules = $this->OldPages->ContentModules->find()->where(['refscope' => 'Content.Pages', 'refid' => $id]);
        //debug($sectionsModules);

        $availableModules = $this->OldPages->ContentModules->Modules->find('list');
    ...
    public function add()
    {
//        $content = $this->OldPages->newEntity();
//        if ($this->request->is('post')) {
//            $content = $this->OldPages->patchEntity($content, $this->request->data);
//            if ($this->OldPages->save($content)) {
...
//        $this->set('_serialize', ['content']);
    ...
    {
        $contentModule = $this->OldPages->ContentModules->newEntity(
            ['refscope' => 'Content.Pages', 'refid' => $id],
            ['validate' => false]
        );
        if ($this->request->is(['post', 'put'])) {
            $this->OldPages->ContentModules->patchEntity($contentModule, $this->request->data);
            if ($this->OldPages->ContentModules->save($contentModule)) {
